...
Feature: Lesen der Ressource Practitioner (@Practitioner-Read)
  ...
  Scenario: Vorbedingung
    ...
    """
      ...
      Telematik-ID: 123456789 (falls vom System unterstützt)
  ...
  Scenario: Read einer Practitioner-Ressource anhand der ID
    ...
    And FHIR current response body evaluates the FHIRPath "identifier.where(system='https://gematik.de/fhir/sid/telematik-id').exists().not() or identifier.where(system='https://gematik.de/fhir/sid/telematik-id' and value = '123456789').exists()" with error message 'Die Telematik-ID entspricht nicht dem Erwartungswert'

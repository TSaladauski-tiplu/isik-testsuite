...
Feature: POST eines Document-Bundles (@Composition-Post)
  ...
  Scenario: POST eines DocumentBundles mit bekannten Patienten und Encounter
    ...
    When TGR send POST request to "http://fhirserver/" with body "!{file('src/test/resources/features/Stufe3/Basis/fixtures/Composition-Post-CorrectCompositionBundle.json')}"
  ...
  Scenario Outline: POST eines inkorrekten DocumentBundles
    Given TGR set default header "Content-Type" to "application/fhir+json"
    When TGR send POST request to "http://fhirserver/" with body "!{file('src/test/resources/features/Stufe3/Basis/fixtures/<inputFile>')}"
    And TGR find the last request

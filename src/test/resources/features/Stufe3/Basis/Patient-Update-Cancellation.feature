...
Feature: Update Patient (@Patient-Update-Cancellation)
  ...
  Scenario: Stornierung eines Patienten durch Update
    ...
    When TGR send PUT request to "http://fhirserver/Patient/${data.patient-update-cancellation-id}" with body "!{file('src/test/resources/features/Stufe3/Basis/fixtures/Patient-Update-Cancellation-Inactive-Fixture.json')}"

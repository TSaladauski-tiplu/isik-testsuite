...
Feature: UPDATE einer Communication-Ressource (@Communication-Update)
  ...
  Scenario: UPDATE einer Communication-Ressource
    ...
    And TGR send PUT request to "http://fhirserver/Communication/${data.communication-update-id}" with body "!{file('src/test/resources/features/Stufe3/Terminplanung/fixtures/Communication-Update-Fixture.json')}"

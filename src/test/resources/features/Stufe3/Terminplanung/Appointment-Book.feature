...
Feature: Buchung eines Termins (@Appointment-Book)
  ...
  Scenario: Vorbedingung
    ...
    """
      - Ein freier Terminblock mit Startzeit 1.1.2027 09:00 und Endzeit 1.1.2027 20:00 muss zuvor manuell im System im beliebigen Kalender angelegt worden sein (bitte die Terminblock-ID in der Konfigurationsvariable 'appointment-book-slot-id' hinterlegt)
  ...
  Scenario: Buchung eines Termins anhand eines freien Terminblocks
    ...
    And TGR send POST request to "http://fhirserver/Appointment/$book" with body "!{file('src/test/resources/features/Stufe3/Terminplanung/fixtures/Appointment-Appointment-Book-Fixture.json')}"
  ...
  Scenario: Buchung eines Termins falls die Terminanfrage unvollständig ist (keine Angaben zum Terminblock bzw. zu einem Kalender)
    Given TGR set default header "Content-Type" to "application/fhir+json"
    And TGR send POST request to "http://fhirserver/Appointment/$book" with body "!{file('src/test/resources/features/Stufe3/Terminplanung/fixtures/Appointment-Appointment-Book-Incomplete-Fixture.json')}"
    Then TGR find the last request
  ...
  @Optional
  Scenario: Buchung eines Termins anhand eines freien Terminblocks, inklusiv Fachrichtung
    Given TGR set default header "Content-Type" to "application/fhir+json"
    And TGR send POST request to "http://fhirserver/Appointment/$book" with body "!{file('src/test/resources/features/Stufe3/Terminplanung/fixtures/Appointment-Appointment-Book-Fixture-With-Specialty.json')}"
    Then TGR find the last request
    And TGR current response with attribute "$.responseCode" matches "20\d"
    #  Asserts for the case if the response is an Appointment
    And FHIR current response body evaluates the FHIRPath '($this is Appointment) or ($this is Parameters)' with error message 'Rückgabe enthält weder eine Appointment noch eine OperationOutcome-Ressource noch eine Parameters-Ressource'

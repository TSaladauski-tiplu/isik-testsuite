...
Feature: Lesen der Ressource Observation (@Observation-Read-Sauerstoffsaettigung)
  ...
  Scenario: Vorbedingung
    ...
    - die ID der korrespondierenden FHIR-Ressourcen zu diesem Testdatensatz muss in der Konfigurationsvariable 'observation-read-sauerstoffsaettigung-id' hinterlegt sein.
  ...
  Scenario: Read einer Observation anhand der ID
    ...
    And resource has ID "${data.observation-read-sauerstoffsaettigung-id}"
    And FHIR current response body is a valid isik3-vitalparameter resource and conforms to profile "https://gematik.de/fhir/isik/v3/VitalparameterUndKoerpermasze/StructureDefinition/ISiKSauerstoffsaettigung"

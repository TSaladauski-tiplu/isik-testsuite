...
@Vitalparameter
@Mandatory
@Observation-Read-Koerpertemperatur
Feature: Lesen der Ressource Observation (@Observation-Read-Koerpertemperatur)
  ...
  Scenario: Vorbedingung
    ...
    - die ID der korrespondierenden FHIR-Ressourcen zu diesem Testdatensatz muss in der Konfigurationsvariable 'observation-read-koerpertemperatur-id' hinterlegt sein.

    Testdatensatz (Name: Wert)
    Erfassen Sie folgende Körpertemperatur:
    Status: Abgeschlossen
    Körpertemperatur: 36.8 Grad Celsisus
  ...
  Scenario: Read einer Observation anhand der ID
    Then Get FHIR resource at "http://fhirserver/Observation/${data.observation-read-koerpertemperatur-id}" with content type "xml"
    And resource has ID "${data.observation-read-koerpertemperatur-id}"
    And FHIR current response body is a valid isik3-vitalparameter resource and conforms to profile "https://gematik.de/fhir/isik/v3/VitalparameterUndKoerpermasze/StructureDefinition/ISiKKoerpertemperatur"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '8310-5' and system = 'http://loinc.org').exists()" with error message 'Der Code entspricht nicht dem Erwartungswert'
    ...
    And FHIR current response body evaluates the FHIRPath "value.where(value ~36.8 and code = 'Cel' and unit.exists() and system = 'http://unitsofmeasure.org').exists()" with error message 'Der Wert der Observation entspricht nicht dem Erwartungswert'

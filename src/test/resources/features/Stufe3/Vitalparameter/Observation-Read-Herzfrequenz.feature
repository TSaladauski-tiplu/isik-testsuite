...
@Vitalparameter
@Mandatory
@Observation-Read-Herzfrequenz
Feature: Lesen der Ressource Observation (@Observation-Read-Herzfrequenz)
  ...
  Scenario: Vorbedingung
    ...
    - die ID der korrespondierenden FHIR-Ressourcen zu diesem Testdatensatz muss in der Konfigurationsvariable 'observation-read-herzfrequenz-id' hinterlegt sein.

    Testdatensatz (Name: Wert)
    Erfassen Sie eine Herzfrequenz mit folgenden Daten:
  ...
  Scenario: Read einer Observation anhand der ID
    Then Get FHIR resource at "http://fhirserver/Observation/${data.observation-read-herzfrequenz-id}" with content type "xml"
    And resource has ID "${data.observation-read-herzfrequenz-id}"
    And FHIR current response body is a valid isik3-vitalparameter resource and conforms to profile "https://gematik.de/fhir/isik/v3/VitalparameterUndKoerpermasze/StructureDefinition/ISiKHerzfrequenz"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '8867-4' and system = 'http://loinc.org').exists()" with error message 'Der Code entspricht nicht dem Erwartungswert'
    ...
    And FHIR current response body evaluates the FHIRPath "value.where(value ~63 and code = '/min' and unit.exists() and system = 'http://unitsofmeasure.org').exists()" with error message 'Der Wert der Observation entspricht nicht dem Erwartungswert'

...
@Vitalparameter
@Mandatory
@Observation-Read-Koerpergewicht
Feature: Lesen der Ressource Observation (@Observation-Read-Koerpergewicht)
  ...
  Scenario: Vorbedingung
    ...
    - die ID der korrespondierenden FHIR-Ressourcen zu diesem Testdatensatz muss in der Konfigurationsvariable 'observation-read-koerpergewicht-id' hinterlegt sein.
  ...
  Scenario: Read einer Observation anhand der ID
    Then Get FHIR resource at "http://fhirserver/Observation/${data.observation-read-koerpergewicht-id}" with content type "xml"
    And resource has ID "${data.observation-read-koerpergewicht-id}"
    And FHIR current response body is a valid isik3-vitalparameter resource and conforms to profile "https://gematik.de/fhir/isik/v3/VitalparameterUndKoerpermasze/StructureDefinition/ISiKKoerpergewicht"
    ...
    And FHIR current response body evaluates the FHIRPath "code.coding.where(code = '29463-7' and system = 'http://loinc.org').exists()" with error message 'Der Code entspricht nicht dem Erwartungswert'
    ...
    And FHIR current response body evaluates the FHIRPath "value.where(value ~79 and code = 'kg' and unit.exists() and system = 'http://unitsofmeasure.org').exists()" with error message 'Der Wert der Observation entspricht nicht dem Erwartungswert'

...
Feature: Lesen der Ressource MedicationStatement (Extended) (@MedicationStatement-Read-Extended)
  ...
  Scenario: Read eines Medikaments anhand der ID
    ...
    And FHIR current response body evaluates the FHIRPath "dosage.first().timing.repeat.when = 'PC'" with error message 'Die Dosierungsangabe Wiederholung entspricht nicht dem Erwartungswert'
    And FHIR current response body evaluates the FHIRPath "dosage.first().timing.repeat.frequency = 2 and dosage.first().timing.repeat.frequencyMax = 3 and dosage.first().timing.repeat.period = 1 and dosage.first().timing.repeat.periodUnit = 'd'" with error message 'Die Dosierungsangabe Frequenz entspricht nicht dem Erwartungswert'
    And FHIR current response body evaluates the FHIRPath "dosage.first().asNeeded = false"
    And FHIR current response body evaluates the FHIRPath "dosage.first().site.coding.where(system='http://snomed.info/sct' and code='738956005').exists()" with error message 'Die Dosierungsangabe Körperstelle entspricht nicht dem Erwartungswert'
    And FHIR current response body evaluates the FHIRPath "dosage.first().route.coding.where(system='http://snomed.info/sct' and code='26643006').exists()" with error message 'Die Dosierungsangabe Route entspricht nicht dem Erwartungswert'
    And FHIR current response body evaluates the FHIRPath "dosage.first().maxDosePerPeriod.numerator.value ~ 600 and dosage.first().maxDosePerPeriod.numerator.system = 'http://unitsofmeasure.org' and dosage.first().maxDosePerPeriod.numerator.code = 'mg' and dosage.first().maxDosePerPeriod.numerator.unit = 'mg' and dosage.first().maxDosePerPeriod.denominator.value ~ 1 and dosage.first().maxDosePerPeriod.denominator.system = 'http://unitsofmeasure.org' and dosage.first().maxDosePerPeriod.denominator.code = 'd' and dosage.first().maxDosePerPeriod.denominator.unit = 'Tag'" with error message 'Die Dosierungsangabe Maximaldosis pro Zeitraum entspricht nicht dem Erwartungswert'
    And FHIR current response body evaluates the FHIRPath "dosage.first().maxDosePerAdministration.value ~ 200 and dosage.first().maxDosePerAdministration.system = 'http://unitsofmeasure.org' and dosage.first().maxDosePerAdministration.code = 'mg' and dosage.first().maxDosePerAdministration.unit = 'mg'" with error message 'Die Dosierungsangabe Maximaldosis pro Zeitraum entspricht nicht dem Erwartungswert'

...
Feature: Lesen der Ressource MedicationAdministration (Rate) (@MedicationAdministration-Read-Rate)
  ...
  Scenario: Read einer MedicationAdministration mit Rateangaben anhand der ID
    ...
    And FHIR current response body evaluates the FHIRPath "dosage.dose.code = 'mL' and dosage.dose.system = 'http://unitsofmeasure.org' and dosage.dose.unit.hasValue() and dosage.dose.value ~ 1000" with error message "Die Gesamtdosis entspricht nicht dem Erwartungswert"
    And FHIR current response body evaluates the FHIRPath "(dosage.rate.is(Quantity) and (dosage.rate.code = 'mL/h' and dosage.rate.system = 'http://unitsofmeasure.org' and dosage.rate.unit.hasValue() and dosage.rate.value ~ 50 ) ) or ( dosage.rate.is(Ratio) and ( dosage.rate.numerator.value ~ 50 and dosage.rate.numerator.unit = 'mL' and dosage.rate.numerator.code = 'mL' and dosage.rate.numerator.system = 'http://unitsofmeasure.org' and dosage.rate.denominator.value ~ 1 and dosage.rate.denominator.unit = 'h' and dosage.rate.denominator.code = 'h' and dosage.rate.denominator.system = 'http://unitsofmeasure.org') )" with error message "Die Verabreichungsrate entspricht nicht dem Erwartungswert"

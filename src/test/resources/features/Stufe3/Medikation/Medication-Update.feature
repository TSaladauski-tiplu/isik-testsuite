...
Feature: Update einer Medication-Ressource (@Medication-Update)
  ...
  Scenario: Update einer Medication-Ressource
    ...
    When TGR send PUT request to "http://fhirserver/Medication/${data.medication-update-id}" with body "!{file('src/test/resources/features/Stufe3/Medikation/fixtures/Medication-Update-Fixture.json')}"
    And TGR find the last request
    Then TGR current response with attribute "$.responseCode" matches "200"
    And TGR send empty GET request to "http://fhirserver/Medication/${data.medication-update-id}"
    And TGR find the last request
    And FHIR current response body evaluates the FHIRPath "amount.numerator.value ~ 40" with error message 'Die Menge wurde nicht oder nicht richtig aktualisiert'

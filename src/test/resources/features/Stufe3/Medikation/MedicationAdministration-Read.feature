...
Feature: Lesen der Ressource MedicationAdministration (@MedicationAdministration-Read)
  ...
  Scenario: Vorbedingung
    ...
    """
      ...
      Grund der Medikation: Beliebig, entweder als Referenz auf eine ISiKDiagnose (bitte ID in der Konfigurationsvariable 'medication-condition-id' eingeben) oder als Code
      Notiz: Testnotiz
  ...
  Scenario: Read eines Medikaments anhand der ID
    ...
    And FHIR current response body evaluates the FHIRPath "dosage.where(text.empty().not() and site.coding.where(code = '738956005' and system = 'http://snomed.info/sct' and display = 'Oral').exists() and dose.where(code = '1' and system = 'http://unitsofmeasure.org' and unit = 'Brausetablette' and value ~ 1).exists() and route.coding.where(code = '26643006' and system = 'http://snomed.info/sct').exists()).exists()" with error message 'Die Dosis entspricht nicht dem Erwartungswert'

    # Achtung! Die folgende Assertion lädt ggf. eine neue Ressource in den Kontext herunter. Nachfolgende Assertions sollen berücksichtigen, dass nicht mehr die ursprüngliche MedicationAdministration-Ressource im Kontext ist.

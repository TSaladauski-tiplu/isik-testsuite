...
Feature: Lesen der Ressource MedicationStatement (Rate) (@MedicationStatement-Read-Rate)
  ...
  Scenario: Read eines Medikaments anhand der ID
    ...
    And FHIR current response body evaluates the FHIRPath "dosage.where(text.empty().not() and doseAndRate.dose.where(code = 'mL' and system = 'http://unitsofmeasure.org' and unit.hasValue() and value ~ 1000).exists()).exists()" with error message 'Die Dosis entspricht nicht dem Erwartungswert'
